...
@Getter 
@Setter
public class ReporteAcademicoNpsAction extends ActionSupport {
	...
	@Action(value = "/reporteAcademicoNpsExcel", results = {
	...
	public String listaReporteAcademicoNps() {
		...
		try {
			...
			fileName = "Reporte_NPS_Academico_" + sdf.format(d) + ".xlsx";
			log.info("fileName : " + fileName);

			String titulo = "Reporte de avance NPS Académico ";

			String titulo_Sheet = "NPS Académico";
			...
			// FILA 1: Se crea la fila en blanco
			...
			List<ReporteAvanceAcademicoNps> lista = model.listaReporteNpsAcademico();

			log.info("lista.sise : " + lista.size());
			ReporteAvanceAcademicoNps bean = null;
			XSSFRow filaX = null;
			XSSFCell celAvance = null,celProPor = null,celNeuPor = null,celDetPor = null;
			for (int i = 0; i < lista.size(); i++) {
				bean = lista.get(i);
				filaX = hoja.createRow(i + 3);
				filaX.createCell(0).setCellValue(bean.getSede());
				filaX.createCell(1).setCellValue(bean.getCarrera());
				filaX.createCell(2).setCellValue(bean.getModalidad());
				
				filaX.createCell(3).setCellValue(bean.getUniverso());
				filaX.createCell(4).setCellValue(bean.getEncuestados());
				
				celAvance = filaX.createCell(5);
				celAvance.setCellValue(bean.getAvance());
				celAvance.setCellStyle(estiloPorcentaje);

				filaX.createCell(6).setCellValue(bean.getPromotores());
				filaX.createCell(7).setCellValue(bean.getNeutros());
				filaX.createCell(8).setCellValue(bean.getDetractores());
				
				celProPor = filaX.createCell(9);
				celProPor.setCellValue(bean.getPromotoresPorcentaje());
				celProPor.setCellStyle(estiloPorcentaje);
				
				celNeuPor = filaX.createCell(10);
				celNeuPor.setCellValue(bean.getNeutrosPorcentaje());
				celNeuPor.setCellStyle(estiloPorcentaje);
				
				celDetPor = filaX.createCell(11);
				celDetPor.setCellValue(bean.getDetractoresPorcentaje());
				celDetPor.setCellStyle(estiloPorcentaje);
				
				filaX.createCell(12).setCellValue(bean.getNps());
			}
