...
@Getter 
@Setter
public class ListadoAcademicoNpsAction extends ActionSupport {
	...
	@Action(value = "/listadoAcademicoNpsExcel", results = {
			@Result(name = SUCCESS, type = "stream", params = {
			"contentType", "application/vnd.ms-excel", 
			"inputName", "inputStream", 
			"contentDisposition","attachment;filename=${fileName}",
			"contentLength", "${fileSize}", }) })
	public String listadoAcademicoNpsExcel() {
		log.info("En metodo: listadoAcademicoNpsExcel");

		NpsModel model = new NpsModel();

		String[] columnText_sheet_1 = { "ID Alumno"," NPS","Comentario","Campus","Ciudad",
									"Carrera","Facultad","Semestre","Ciclo","Tipo de Estudiante",
									" Fecha de Registro","Modalidad","Estado" };

		int[] columnWith_sheet_1 = { 3000, 2000, 10000, 4000, 4000,
									15000, 15000, 4000, 4000, 10000 ,
									 5000 ,3700, 3000 };
		...
		try {
			...
			fileName = "Listado_NPS_Academico_" + sdf.format(d) + ".xlsx";
			log.info("fileName : " + fileName);

			String titulo_1 = "Listado de NPS Académico";
			...
			List<ReporteListaAcademicoNps> lista = model.listaAlumnposNpsAcademico();

			log.info("lista.sise : " + lista.size());
			ReporteListaAcademicoNps bean = null;
			XSSFRow filaX = null;
			for (int i = 0; i < lista.size(); i++) {
				...
				filaX.createCell(4).setCellValue(bean.getCiudad());
				filaX.createCell(5).setCellValue(bean.getCarrera());
				filaX.createCell(6).setCellValue(bean.getFacultad());
				filaX.createCell(7).setCellValue(bean.getSemestre());
				filaX.createCell(8).setCellValue(bean.getCiclo());
				filaX.createCell(9).setCellValue(bean.getTipoEstudiante());
				filaX.createCell(10).setCellValue(bean.getFechaRegistro());
				filaX.createCell(11).setCellValue(bean.getModalidad());
				filaX.createCell(12).setCellValue(bean.getEstado());
