...
@Getter 
@Setter
public class ListadoMensajeExtensionNpsAction extends ActionSupport {
	...
	@Action(value = "/listadoMensajeExtensionNpsExcel", results = {
	...
	public String listadoExtensionNpsExcel() {
		...
		String[] columnText_sheet_1 = { "Negocio","Campus","Escuela", "Familia",
										"Curso","Modalidad","ID Alumno","ID Mensaje"," Comentario",
										"NPS","Pregunta","Aspecto","Tema" ,
										"Tipo","Valoración", "Usuario"};

		int[] columnWith_sheet_1 = { 3000, 3000, 6000, 6000, 
									 15000, 6000, 4000, 3000, 20000,
									 3000 , 15000, 10000 ,15000,
									 4000, 3000, 10000};
		...
		try {
			...
			fileName = "Listado_Valoracion_Mensaje_NPS_Extension_" + sdf.format(d) + ".xlsx";
			log.info("fileName : " + fileName);

			String titulo_1 = "Listado de valoración de mensaje NPS Extensión";
			...
			List<ReporteListaMensajeExtensionNps> lista = model.listaMensajeNpsExtension();

			log.info("lista.sise : " + lista.size());
			ReporteListaMensajeExtensionNps bean = null;
			XSSFRow filaX = null;
			for (int i = 0; i < lista.size(); i++) {
				bean = lista.get(i);
				filaX = hoja.createRow(i + 3);
				filaX.createCell(0).setCellValue(bean.getNegocio());
				filaX.createCell(1).setCellValue(bean.getCampus());
				filaX.createCell(2).setCellValue(bean.getEscuela());
				filaX.createCell(3).setCellValue(bean.getFamilia());
				...
				celda5.setCellValue(bean.getModalidad());
				
				XSSFCell celda6 = filaX.createCell(6);
				celda6.setCellStyle(estiloCeldaCentradoSimple);
				celda6.setCellValue(bean.getIdAlumno());
				
				XSSFCell celda7 = filaX.createCell(7);
				celda7.setCellStyle(estiloCeldaCentradoSimple);
				celda7.setCellValue(bean.getIdMensaje());
				
				filaX.createCell(8).setCellValue(bean.getComentario());
				
				XSSFCell celda9 = filaX.createCell(9);
				celda9.setCellStyle(estiloCeldaCentradoSimple);
				celda9.setCellValue(bean.getNps());
				
				filaX.createCell(10).setCellValue(bean.getPregunta());
				filaX.createCell(11).setCellValue(bean.getAspecto());
				filaX.createCell(12).setCellValue(bean.getTema());
				
				filaX.createCell(13).setCellValue(bean.getTipo());
				filaX.createCell(14).setCellValue(bean.getValoracion());
				filaX.createCell(15).setCellValue(bean.getUsuario());
			}
