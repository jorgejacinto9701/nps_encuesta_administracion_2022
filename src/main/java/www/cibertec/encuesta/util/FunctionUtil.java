...
public class FunctionUtil {
	...
	public static final String FORMULA = "((([0-9]{1,3}|([0-9]+[.][0-9]+))[%][(][A-Z]{2}[1-9][)])([+]?))+";
	public static final String CLAVE_PERMITIDA = "^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=\\S+$).{8,20}$";
	...
	public static boolean isOperacion(String operacion) {
		if (operacion.matches(FORMULA)) {
			return true;
		}else {
			return false;
		}
	...
	public static List<Curso> listaCursoActivosOrdenadoPorNombreAsc(List<Curso> lista) {
		Collections.sort(lista, new Comparator<Curso>() {
			@Override
			public int compare(Curso o1, Curso o2) {
				String curso1 = null, curso2 = null;
				try {
					String[] sep  = o1.getNombre().split("-");
					curso1 = sep[sep.length-1].trim();
				} catch (Exception e) {
					curso1 = o1.getNombre();
				}
				try {
					String[] sep  = o2.getNombre().split("-");
					curso2 = sep[sep.length-1].trim();
				} catch (Exception e) {
					curso2 = o2.getNombre();
				} 
				return curso1.compareToIgnoreCase(curso2);
			}
		});
		return lista;
	}
